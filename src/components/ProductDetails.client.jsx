...
export default function ProductDetails({product}) {
  ...
  const previousPrice = product?.compareAtPriceRange?.maxVariantPrice?.amount;
  const discountPercentageText =
    previousPrice > 0 && Math.floor((price / previousPrice) * 100) + '%';
    ...
    <>
      <ProductProvider data={product} initialVariantId={initialVariant.id}>
        <div className="grid grid-cols-1 md:grid-cols-[1fr,1fr] gap-x-8 my-16">
          ...
          <div className="p-4">
            {product.vendor && (
              <div className="text-sm tracking-[0.25rem] font-extrabold mb-2 text-orange-400">
                {product.vendor}
              </div>
            )}
            <ProductTitle
            ...
            <div className="flex items-center">
              ...
              />
              {previousPrice > 0 && (
                <div className="mx-2 px-2 pb-1 font-bold rounded-md bg-orange-100 text-orange-600">
